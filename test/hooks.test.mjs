...
const CORE_RULES = {
  ...
  OPERATOR_RULES: [OPERATOR_RULES, [/"arcade" MCP server/, /api\.arcade\.dev/, /return needs_auth/, /return failed/, /needsAuth/]],
  SUBAGENT_CONTEXT: [SUBAGENT_CONTEXT, [/"arcade" MCP server/, /api\.arcade\.dev/, /try-arcade/, /return needs_auth/, /needsAuth/, /Keep tool discovery/]],
};

// Arcade is the first choice for app tasks, not the only one. When Arcade
// can't do part of a task, the agent may finish it with gh, curl, a built-in
// search, or another MCP server without asking first.
