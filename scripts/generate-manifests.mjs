...
// A table entry names one event, several events, or none for this host.
const hooksForHost = (hostName) => {
  const found = [];
  for (const hook of HOOKS) {
    const events = hook.events[hostName] ?? [];
    for (const event of Array.isArray(events) ? events : [events]) {
      found.push({ event, command: hookCommand(hostName, hook.script) });
    }
  }
  return found;
};
...
const buildClaudeHooks = () => {
  ...
  for (const { event, command } of hooksForHost("claude-code")) {
    ...
    // Scripts that share an event run from the same entry.
    hooks[event] ??= [{ ...(matcher ? { matcher } : {}), hooks: [] }];
    hooks[event][0].hooks.push({ type: "command", command, timeout: HOOK_TIMEOUT_SEC });
  }
  return { hooks };
};
...
const buildCursorHooks = () => {
  ...
  for (const { event, command } of hooksForHost("cursor")) {
    hooks[event] ??= [];
    hooks[event].push({ command, timeout: HOOK_TIMEOUT_SEC });
  }
