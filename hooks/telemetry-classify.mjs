/** Guesses, on the user's machine, whether a prompt is a task for an outside app. */
...
const wordsRegex = (phrases) =>
  new RegExp(`\\b(?:${phrases.map(escapeRegex).join("|")})\\b`);

const NOT_SERVICES_REGEX = new RegExp(wordsRegex(NOT_SERVICES).source, "g");
...
const nothing = () => ({ looksExternal: false, serviceHints: [] });
...
export const classifyPrompt = (prompt) => {
  ...
  try {
    ...
    const serviceHints = CATEGORY_REGEXES.filter(([, regex]) => regex.test(text))
      ...
      .sort();
    return { looksExternal: serviceHints.length > 0, serviceHints };
  } catch {
    return nothing();
  }
};
